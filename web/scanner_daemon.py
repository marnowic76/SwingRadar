...
class SwingRadarDaemon:
    ...
    def run_cycle(self):
        ...
        if regime == "BEAR":
            self.stability.MIN_MEAN_SCORE = 70.0
            logger.info("Market Regime BEAR — podnoszę próg stabilności do 70")
        elif regime == "BULL":
            self.stability.MIN_MEAN_SCORE = 55.0
            logger.info("Market Regime BULL — obniżam próg stabilności do 55")
        else:
            self.stability.MIN_MEAN_SCORE = 60.0
            logger.info("Market Regime NEUTRAL — standardowy próg stabilności 60")

        # Zapisz regime do bazy żeby generator HTML mógł go odczytać
        try:
            with sqlite3.connect("data/trading_system.db") as _rc:
                _rc.execute("""
                    CREATE TABLE IF NOT EXISTS market_regime_log (
                        id         INTEGER PRIMARY KEY AUTOINCREMENT,
                        regime     TEXT NOT NULL,
                        sectors_up INTEGER DEFAULT 0,
                        sectors_dn INTEGER DEFAULT 0,
                        updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
                    )""")
                _rc.execute("""
                    INSERT INTO market_regime_log (regime, sectors_up, sectors_dn)
                    VALUES (?, ?, ?)""",
                    (regime,
                     regime_data.get("sectors_up", 0),
                     regime_data.get("sectors_dn", 0)))
                # Keep only last 100 records
                _rc.execute("""
                    DELETE FROM market_regime_log WHERE id NOT IN (
                        SELECT id FROM market_regime_log ORDER BY id DESC LIMIT 100
                    )""")
        except Exception as _re:
            logger.warning(f"Could not save regime to DB: {_re}")
